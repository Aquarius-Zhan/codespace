...
StackResult stackPush(Stack* stack, char bracket, int position){
    ...
    if (stack->top + 1 == stack->capacity)
    {
        ...
        {
            if ((stack->data = (StackElem*) realloc(stack->data, (stack->capacity + 1) * sizeof(StackElem))) == NULL)
            {
                return STACK_FULL;
            }
            else
...
StackResult stackPop(Stack* stack, StackElem* elem){
    ...
    // 步骤3：栈顶指针-1
    // 步骤4：返回STACK_OK
    if (stackIsEmpty(stack) == 1) return STACK_EMPTY;
...
int jsonBracketCheckBasic(const char *json_str) {
    ...
    Stack stack;
    stackInit(&stack, 2, strlen(json_str));

    for (int i = 0; i < strlen(json_str); i ++)
    {
        if (isLeftBracket(*(json_str + i)) == 1)
        {
            stackPush(&stack, *(json_str + i), i);
            continue;
        }
        if (isRightBracket(*(json_str + i)) == 1)
        {
            char right = *(json_str + i);
            char left = ((&stack)->data + (&stack)->top)->bracket;
            stackPop((&stack), (&stack)->data + (&stack)->top);
            if (isBracketMatch(left, right) == 0) 
            {
                stackDestroy(&stack);
                return 0;
            }
...
// 步骤3：初始化状态标记（in_string标记字符串状态，escape标记转义状态）
    int in_string = 0;
    //标记是否在字符串内（0/1）
    int escape = 0;//转义标记（0/1，仅对下一字符生效）
// 步骤4：遍历json_str的每个字符：
    for (int i = 0; i < strlen(json_str); i ++)
    {
//        - 如果在字符串内（in_string为1）：
