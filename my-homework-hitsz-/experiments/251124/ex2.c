...
int oaInsertKeepOrder(Customer customers[], int n, int capacity, Customer c, Metrics *m) {
    ...
    for (int i = 0; i < n; ++i) {
        if (m) m->compares++;
        if (customers[i].id >= c.id) { pos = i; break; }
    }

    if (pos < n) {
        memmove(&customers[pos + 1], &customers[pos], (size_t)(n - pos) * sizeof(Customer));
        if (m) m->moves += (n - pos); /* 每个元素算一次移动 */
    }
    customers[pos] = c;
    if (m) m->moves++;
